...
1. **Rewards must be claimable**: The Rollup owner enables claiming via `setRewardsClaimable(true)` after a governance decision or after the earliest claimable timestamp passes.
...
This returns a Unix timestamp indicating the earliest time when the owner can enable reward claiming.
...
**Example:**
```bash
cast call $ROLLUP_ADDRESS "getSequencerRewards(address)" 0x742d35Cc6634C0532925a3b844Bc9e7595f0bEb --rpc-url $RPC_URL
# Output: 0x000000000000000000000000000000000000000000000000016345785d8a0000

# Convert to decimal tokens (assuming 18 decimals)
cast --from-wei $(cast --to-dec 0x000000000000000000000000000000000000000000000000016345785d8a0000)
# Output: 0.1
```

## Claiming Your Rewards

The `claimSequencerRewards` function is permissionless - anyone can call it for any address. Rewards are always sent to the specified recipient, regardless of who submits the transaction.
...
Replace:
- `[RECIPIENT_ADDRESS]` - The address that will receive the rewards
...
cast call $ROLLUP_ADDRESS "getSequencerRewards(address)" [RECIPIENT_ADDRESS] --rpc-url $RPC_URL
```

## Delegated Stake Considerations

If you're operating with delegated stake, your coinbase is configured as a [Split contract](https://docs.splits.org/core/split) that distributes rewards between you and your delegators.

Claim to the Split contract address:
...
After claiming, trigger distribution from the Split contract according to the configured shares. See the [0xSplits documentation](https://docs.splits.org/core/split) for distribution commands and [Running Delegated Stake](./running_delegated_stake.md) for setup details.
...
2. If `false`, wait until the Rollup owner enables claiming via `setRewardsClaimable(true)`
3. Check the earliest claimable timestamp using `getEarliestRewardsClaimableTimestamp()`
4. Contact the network operators or check governance proposals for information about when claiming will be enabled
...
1. Check your account balance:
   ...
   ```
2. Send ETH to your account to cover gas costs (recommended: at least 0.01 ETH)
