...
**Key parameters** (ignition network):
...
**Proposed penalty** (ignition network): 1% of stake
...
**Proposed penalty** (ignition network): 0% (disabled for initial deployment)
...
# Epoch prune and data withholding penalties (disabled by default for ignition)
...
**Automatic protection**: Your own validator addresses (from your keystore) are automatically added to `SLASH_VALIDATORS_NEVER` unless you set `slashSelfAllowed=true` via the API.
...
**Temporary Disable**: The vetoer can disable all slashing for up to 5 days if needed, with the ability to extend this period.
...
**Ejection Threshold** (Ignition network): 98% of Activation Threshold
...
**Solutions**:
1. For Ignition network, all penalties should be set to `2000000000000000000000` (2000 tokens, 1%)
