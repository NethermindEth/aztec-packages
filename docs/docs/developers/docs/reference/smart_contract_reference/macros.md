...
- `#[only_self]` - Function can only be called from within the contract and the call itself is not inlined (e.g. not akin to EVM's JUMP but instead EVM's CALL)
- `#[internal]` - NOTE YET IMPLEMENTED - Function can only be called from within the contract and the call itself is inlined (e.g. akin to EVM's JUMP and not EVM's CALL)
