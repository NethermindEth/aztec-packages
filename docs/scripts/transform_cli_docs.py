...
Usage:
    python transform_cli_docs.py --input docs.json --output cli-reference.md
    python transform_cli_docs.py --input docs.yaml --output cli-reference.md --template custom_template.py
"""
...
class MarkdownGenerator:
    ...
    def render_commander_command(self, cmd_data: Dict[str, Any], depth: int) -> str:
        ...
        if cmd_data.get("description"):
            sections.append(cmd_data["description"] + "\n")
        ...
        if cmd_data.get("commands"):
            ...
            for cmd in cmd_data["commands"]:
                sections.append(f"- `{cmd['signature']}` - {cmd['description']}")
            sections.append("")
