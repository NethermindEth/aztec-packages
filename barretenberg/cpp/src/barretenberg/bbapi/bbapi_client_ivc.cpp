...
ClientIvcStart::Response ClientIvcStart::execute(BBApiRequest& request) &&
{
    ...
    request.ivc_in_progress = std::make_shared<ClientIVC>(num_circuits);
...
ClientIvcProve::Response ClientIvcProve::execute(BBApiRequest& request) &&
{
    ...
    auto sumcheck_ivc = std::dynamic_pointer_cast<ClientIVC>(request.ivc_in_progress);
    ...
    response.proof =
        ClientIVC::Proof{ .mega_proof = std::move(proof.mega_proof), .goblin_proof = std::move(proof.goblin_proof) };
