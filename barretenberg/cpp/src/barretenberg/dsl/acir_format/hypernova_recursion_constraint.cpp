...
std::shared_ptr<ClientIVC> create_mock_sumcheck_ivc_from_constraints(
    const std::vector<RecursionConstraint>& constraints)
{
    auto ivc = std::make_shared<ClientIVC>(constraints.size());
    ...
    if (constraints.size() == 2) {
        ...
        mock_sumcheck_ivc_accumulation(ivc, ClientIVC::QUEUE_TYPE::PG, /*is_kernel=*/true);
        mock_sumcheck_ivc_accumulation(ivc, ClientIVC::QUEUE_TYPE::PG, /*is_kernel=*/false);
        return ivc;
    }
    ...
    if (constraints.size() == 1 && constraints[0].proof_type == pg_final_type) {
        mock_sumcheck_ivc_accumulation(ivc, ClientIVC::QUEUE_TYPE::PG_FINAL, /*is_kernel=*/true);
        return ivc;
    }
 ...
 *
 */
ClientIVC::VerifierInputs create_mock_verification_queue_entry_nova(const ClientIVC::QUEUE_TYPE verification_type,
                                                                    const bool is_kernel)
{
    using IvcType = ClientIVC;
    using FF = IvcType::FF;
    ...
    if (is_kernel) {
        ...
        BB_ASSERT_EQ(verification_type == ClientIVC::QUEUE_TYPE::PG ||
                         verification_type == ClientIVC::QUEUE_TYPE::PG_TAIL ||
        ...
        BB_ASSERT_EQ(verification_type == ClientIVC::QUEUE_TYPE::OINK || verification_type == ClientIVC::QUEUE_TYPE::PG,
                     true);

        // The first app is not folded thus the proof does not include the nova fold proof
        bool include_fold = !(verification_type == ClientIVC::QUEUE_TYPE::OINK);
 ...
 */
void mock_sumcheck_ivc_accumulation(const std::shared_ptr<ClientIVC>& ivc,
                                    ClientIVC::QUEUE_TYPE type,
                                    const bool is_kernel)
{
    using FF = ClientIVC::FF;
    using Commitment = ClientIVC::Commitment;

    // Initialize verifier accumulator with proper structure
    ivc->recursive_verifier_native_accum.challenge = std::vector<FF>(ClientIVC::Flavor::VIRTUAL_LOG_N, FF::zero());
    ...
    ClientIVC::VerifierInputs entry = acir_format::create_mock_verification_queue_entry_nova(type, is_kernel);
