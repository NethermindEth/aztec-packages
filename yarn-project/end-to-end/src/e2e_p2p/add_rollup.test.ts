...
import { AztecAddress, EthAddress, Fr, generateClaimSecret, retryUntil, sleep, waitForProven } from '@aztec/aztec.js';
  ...
  it('Should cast votes to add new rollup to registry', async () => {
    ...
    const bridging = async (
      ...
      // Then we consume the L2 -> L1 message
      {
        ...
        const epoch = await rollup.getEpochNumberForBlock(l2OutgoingReceipt.blockNumber!);
