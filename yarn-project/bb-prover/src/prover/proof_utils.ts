...
import { ChonkProof, Proof, RecursiveProof } from '@aztec/stdlib/proofs';
 ...
 */
export async function writeChonkProofToPath(chonkProof: ChonkProof, outputPath: string) {
  // NB: Don't use chonkProof.toBuffer here because it will include the proof length.
  const fieldsBuf = Buffer.concat(chonkProof.proof.map(field => field.toBuffer()));
