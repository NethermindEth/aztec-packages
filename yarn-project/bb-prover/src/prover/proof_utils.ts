...
import { ClientIvcProofWithPI, Proof, RecursiveProof } from '@aztec/stdlib/proofs';
...
export async function readClientIVCProofFromOutputDirectory(directory: string) {
  ...
  return new ClientIvcProofWithPI(proofFields);
}
 ...
 */
export async function writeClientIVCProofToPath(clientIvcProof: ClientIvcProofWithPI, outputPath: string) {
